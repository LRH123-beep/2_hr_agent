...
atexit.register(close_db,db_conn)   # 注册进程退出时间的清理钩子hook，只要不断电、kill-9 都可以自动关闭数据资源数据库
...
def get_employee_profile(uid:str) -> str:
    ...
    res = query_db(conn=db_conn,sql=sql,params=(uid,)) # 加逗号是因为可以变成元组
...
def get_leave_balance(uid:str) -> str:
    ...
    if not res:
        return f'[错误]：未找到 UID为　{uid} 的假期数据。'
...
def generate_employment_certificate(uid:str,cer_type:str) -> str:
    ...
    if not emp_res:
        return f'[错误]因无法核实员工身份(UID:{uid}),证明生成失败'
    ...
    if cer_type == 'income':
        ...
        if rank_level < 5:
            return (f'[系统提示]根据公司规定，P4 及以下职级员工(当前员工为{employee['rank']})无法线上开具薪资收入证明。'
                    f'请引导员工在线提交人工工单，由HR线下手动核实开具'
                    )
        content = (f'<薪资收入证明>\n 兹证明我公司员工 {employee['name']},职级为{employee['rank']}\n'
                   f'该员工基本薪资为人民币{employee['base_salary']}元\n 特此证明（公章）'
                   )

        # 打印PDF格式如果需要保存文件,可已使用 pathlib 这个是前面预留的接口
        # file_path = OUTPUT_DIR/f'{employee["name"]}_income_cert.txt'  # 拼接
