"""
第一次的切分器的结果会进入第二个切分器继续切分但要是不满足你设置的值就不切分切分器可再次器分一层用Makdown切分再用滑动窗口切分和顺序切分【'\n\n','\n'】
而用户问题改写等内容是 多角度查询和假设的是给模型多个角度好比较
"""
...
RERANK_MODEL = os.getenv('RERANK_MODEL') or str(Path(__file__).resolve().parent / 'bge-reranker-base')
...
print('正在加载 BGE 重排(Reranker) 模型。。。。。。')
reranker = CrossEncoder(RERANK_MODEL, max_length=512, device='cpu')


#由于llm 用于 Query改写 和 HyDE生成，温度设置为0.7可以激发一定的创造力
llm = ChatOpenAI(
...
def build_ensemble_retriever():
    ...
    # 混合:手动 RRF 倒数秩融合
    def rrf_fusion(bm25_docs, vector_docs, weights=(0.4, 0.6), k=60):
        """RRF 融合：score = weight / (rank + k)，按总分排序去重后返回"""
        scores = {}
        for rank, doc in enumerate(bm25_docs, 1):
            scores[doc.page_content] = scores.get(doc.page_content, 0) + weights[0] / (rank + k)
        for rank, doc in enumerate(vector_docs, 1):
            scores[doc.page_content] = scores.get(doc.page_content, 0) + weights[1] / (rank + k)

        # 按 content 去重后按分数降序排列
        seen = {}
        for doc in bm25_docs + vector_docs:
            seen[doc.page_content] = doc
        ranked = sorted(seen.values(), key=lambda d: scores.get(d.page_content, 0), reverse=True)
        return ranked

    return bm25_retriever, voctor_retriever

print('正在构建混合检索器。。。。。')
retriever = build_ensemble_retriever()  # 启动时初始化 build_ensemble_retriver

# 3.智能扩写 HyDE
class QueryExpansion(BaseModel):
    expanded_queries: list[str]  = Field(description='从不同维度扩写 3 个相关检索词或短语')
    hypothetical_document:str = Field(description='针对该问题的一段假设性，看似专业的官方制度回答片段（可以伪造数字）')
...
def search_hr_policy(query:str) -> str:
    ...
    当用户查询任何关于公司规章制度、差旅报销标准、假期政策、福利等相关信息、必须调用工具
    输入参数query必须是用户原始问题
    """
    #步骤一：获取5个查询变成组成的查询矩阵
    ...
    all_condition_docs = []
    bm25_r, vector_r = retriever  # 解包成两个检索器
    for q in search_queries:
        all_condition_docs.extend(bm25_r.invoke(q))
        all_condition_docs.extend(vector_r.invoke(q))
