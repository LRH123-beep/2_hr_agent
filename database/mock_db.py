...
def init_db(db_path: Path = DB_PATH) -> sqlite3.Connection:
    ...
    # 连接数据库
    conn = sqlite3.connect(str(db_path),check_same_thread=False)  # 关闭并发
    conn.execute('PRAGMA foreign_keys = ON')
    cursor = conn.cursor()    # 游标 比喻手指
    ...
    # 清空旧数据（确保幂等性）#####
    cursor.execute('''delete from employees''')  # 全部删除
